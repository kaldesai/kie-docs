[id='tomcat-jdbc-config_proc']
= Configuring JDBC Web Server data sources on Tomcat Server
...
.Prerequisites
* {PRODUCT} for Tomcat Server is installed.

.Procedure
. Copy the following libraries from the offline Maven repository to the `TOMCAT_HOME/tomcat/lib` folder:
+
[source]
----
...
----
. Copy your database JDBC driver to the `TOMCAT_HOME/tomcat/lib` folder.
. Configure the pooling XA data source in the `TOMCAT_HOME/tomcat/conf/context.xml` file:
