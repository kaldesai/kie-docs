[id='tomcat-zip-install-proc']

= Installing {KIE_SERVER} from ZIP files

{KIE_SERVER} provides the runtime environment for business assets and accesses the data stored in the assets repository (knowledge store). You can use  ZIP files to install {KIE_SERVER} on an existing Tomcat Server 9 or higher server instance.

.Prerequisites
* The following files have been downloaded, as described in <<tomcat-install-download-proc_{context}>>:
...
* A backed-up Tomcat Server 9 or higher server installation is available. The base directory of the Tomcat Server installation is referred to as `_TOMCAT_HOME_`.
...
+
In the following instructions, the directory that contains the extracted `{PRODUCT_INIT}-{PRODUCT_VERSION}-kie-server-jws.zip` file is called `TOMCAT_TEMP_DIR` and the directory that contains the extracted
ifdef::PAM[]
...
. Copy the `TOMCAT_TEMP_DIR/{PRODUCT_INIT}-{PRODUCT_VERSION}-kie-server-jws/kie-server.war` directory to the `_TOMCAT_HOME_/tomcat/webapps` directory.
+
[NOTE]
====
...
. Add users and roles to the `_TOMCAT_HOME_/tomcat/conf/tomcat-users.xml` file. In the following example, `<ROLE_NAME>` is a role supported by {PRODUCT}.
