...
. Open `standalone-full.xml` and under the `<system-properties>` tag, set the following JVM properties:
* `_EAP_HOME_/standalone/configuration/standalone-full.xml` for standalone mode.
