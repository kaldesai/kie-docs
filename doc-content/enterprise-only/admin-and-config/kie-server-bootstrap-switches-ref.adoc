...
|`true`, `false`
|`false`
|If set to `true`, disables the {PRODUCT} DMN support.

|`org.kie.swagger.server.ext.disabled`
|`true`, `false`
|`false`
|If set to `true`, disables the {PRODUCT} swagger documentation support
|===
...
|String
|`Administrator`
|Users with process administrator (*process-admin*) or administrator (*admin*) roles are able to access all the tasks from {KIE_SERVER}.

|`org.jbpm.ht.admin.group`
|String
|`Administrators`
|In order to display all the tasks from {KIE_SERVER}, users with process administrator (*process-admin*) or administrator (*admin*) roles must belong to the `Administrators` group.
|===
endif::PAM[]
