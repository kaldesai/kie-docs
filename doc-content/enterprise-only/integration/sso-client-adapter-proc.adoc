...
====
+
. To enable the JACC manually in the `standalone.xml` or in the file where Elytron is installed, do any of the following tasks:

* Elytron subsystem provides a built-in policy provider based on JACC specification. To create the policy provider and enable the JACC manually, enter the following commands in the management command-line interface (CLI) of {EAP}:
+
--
[source]
----
