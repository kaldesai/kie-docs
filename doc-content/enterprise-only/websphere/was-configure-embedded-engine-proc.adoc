...
ifdef::PAM[]
An embedded engine is a light-weight work flow engine and rule engine that allows you to execute your decisions and business processes. An embedded engine can be part of your {PRODUCT_PAM} application or they can be deployed as a service, for example in the cloud. This section describes how to setup an embedded {PROCESS_ENGINE} or {DECISION_ENGINE} in {WEBSPHERE}.
endif::[]
ifdef::DM[]
A {DECISION_ENGINE} is a light-weight rule engine that allows you to execute your decisions. A {DECISION_ENGINE} can be part of your {PRODUCT_DM} application or they can be deployed as a service, for example in the cloud. This section describes how to setup an embedded {DECISION_ENGINE} in {WEBSPHERE}.
endif::[]
...
ifdef::PAM[]
. If you use a {PROCESS_ENGINE} or {DECISION_ENGINE} with persistence support in your project, you must declare the following hibernate dependencies in the `dependencyManagement` secion of your `pom.xml` file.
+
...
+
ifdef::DM[]
. If you use a {DECISION_ENGINE} with persistence support in your project, you must declare the following hibernate dependencies in the `dependencyManagement` secion of your `pom.xml` file.
+
...
[NOTE]
====
Copy the current version of the `version.org.hibernate-4ee7` property from the {PRODUCT_BA} bill of materials (BOM) Parent file to the project’s `pom.xml` file.
====
