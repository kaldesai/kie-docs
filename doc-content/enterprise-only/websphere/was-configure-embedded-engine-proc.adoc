...
ifdef::DM[]
A {DECISION_ENGINE} is a light-weight rule engine that enables you to execute your decisions and business processes. A decision engine can be part of a {PRODUCT_DM} application or it can be deployed as a service through OpenShift, Kubernetes, and Docker. You can embed {DECISION_ENGINE} in a {PRODUCT_DM} application through the API or as a set of contexts and dependency injection (CDI) services.
endif::[]
