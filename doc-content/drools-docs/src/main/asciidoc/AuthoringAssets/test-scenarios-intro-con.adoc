...
The defined test scenarios in both the designers can be executed in multiple ways, for example you can run the all the available test scenarios at a project level or inside a particular test scenario asset. For more information, see xref:test-designer-run-test-proc[]. Test scenarios are independent, so one scenario cannot affect or modify the other. You can run test scenarios at any time during project development in {CENTRAL}. You do not have to compile or deploy your decision service to run test scenarios.
