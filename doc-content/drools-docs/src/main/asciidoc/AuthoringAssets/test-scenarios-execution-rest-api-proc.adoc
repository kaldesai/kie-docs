...
[source]
----
curl -u 'testadmin:admin1234;' -H "accept: application/xml" -X POST "http://localhost:8080/kie-server/services/rest/server/containers/traffic_1.0.0-SNAPSHOT/scesim" --data '<ScenarioSimulationModel version="1.8">
----
+
Example XML request:
+
