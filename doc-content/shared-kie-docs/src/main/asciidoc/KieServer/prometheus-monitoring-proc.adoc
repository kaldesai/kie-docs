...
* *URL*: Enter the {KIE_SERVER} REST API base URL and metrics endpoint, such as `\http://localhost:8080/services/rest/metrics` (or on Spring Boot, `\http://localhost:8080/rest/metrics`).
...
[source]
----
curl -u 'baAdmin:password@1' -H "Content-Type: application/json" -X GET "http://localhost:8080/services/rest/metrics"
----
