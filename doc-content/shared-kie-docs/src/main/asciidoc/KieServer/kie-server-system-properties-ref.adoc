...
|`true`, `false`
|`false`
|When the property is set to `true`, the {KIE_SERVER} will interpret data in the REST API payload. For example, if a data field is supposed to be a number but it is not a number, you receive an error.
