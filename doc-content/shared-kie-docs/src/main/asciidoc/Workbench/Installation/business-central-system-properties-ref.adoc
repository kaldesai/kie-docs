...
* `org.uberfire.nio.git.ssh.cert.dir`: Location of the `.security` directory where local certificates are stored. Default value: the working directory.
...
* `org.uberfire.metadata.index.dir`: Place where the Lucene `.index` directory is stored. Default value: the working directory
* `org.uberfire.ldap.regex.role_mapper`: Regex pattern used to map LDAP principal names to the application role name. Note that the variable role must be part of the pattern because it is substituted by the application role name when matching a principal value to a role name. Default value: Not used.
...
+
ifdef::DROOLS,JBPM[]
To change one of these system properties in a *WildFly* or *JBoss EAP* cluster, use the following steps:

. Edit the `$JBOSS_HOME/domain/configuration/host.xml` file.
. Locate the `XML` elements that belong to the `main-server-group` and add a system property, for example:
+
[source,xml]
----
<system-properties>
